...
program_revenue = agency_data.groupby("Program_Name")['Total_Amount'].sum().nlargest(10).reset_index()
custom_labels = st.checkbox("Use custom labels")
if custom_labels:
    program_labels = st.multiselect('Select programs', sorted(program_revenue["Program_Name"].unique()))
else:
    program_labels = sorted(program_revenue["Program_Name"].unique())
